...
const CitySearch = () => {
    ...
    const loadSuggestions = async () => {
      try {
        if (city.length > 2 && shouldLoadSuggestions) {
          const response = await axios.get(`https://nikolaymelni.pythonanywhere.com/shelter/autocomplete`, {
            params: { input: city }
          });
  ...
  const handleSearch = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`https://nikolaymelni.pythonanywhere.com/shelter/find_places_in_city`, {
